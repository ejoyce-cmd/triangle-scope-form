...
function generatePDF(address, tech, sections) {
  ...
  sections.forEach(function(section) {
    ...
    allItems.forEach(function(item) {
      ...
      currentPage.push('0 0 0 rg');
      if (cost > 0) {
        currentPage.push('BT /F2 8 Tf 492 ' + (yPos - 10) + ' Td ($' + cost.toFixed(2) + ') Tj ET');
      }
      yPos -= 20;

      if (item.comments) {
        checkY(16);
        currentPage.push('BT /F1 7 Tf 36 ' + (yPos - 8) + ' Td (Notes: ' + esc(item.comments) + ') Tj ET');
        yPos -= 13;
      }

      if (item.photoCount > 0) {
        checkY(14);
        currentPage.push('0.1 0.5 0.8 rg');
        currentPage.push('BT /F1 7 Tf 36 ' + (yPos - 8) + ' Td (' + item.photoCount + ' photo(s) captured) Tj ET');
        currentPage.push('0 0 0 rg');
        yPos -= 13;
      }
    });
  ...
  const pageNums = [];
  contentNums.forEach(function(cNum, idx) {
    const pNum = objNum++;
    pageNums.push(pNum);
...
exports.handler = async function(event) {
  ...
  try {
    ...
    const results = { monday: false, pdf: false, errors: [] };

    // Get Monday column IDs
    ...
    if (itemId) {
      ...
      try {
        ...
        if (pdfRes.status === 200 || (pdfRes.data && pdfRes.data.data)) {
          ...
          results.errors.push('PDF: ' + JSON.stringify(pdfRes.data).substring(0, 100));
        }
      } catch(e) {
